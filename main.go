...
func main() {
	...
	for _, rule := range lib.Config.Rules {
		...
		// mailbox may be a plain name or an IMAP LIST wildcard pattern (e.g. "INBOX.*")
		// matching many folders; expand it up front so one rule can apply to all of them.
		mailboxNames, err := lib.ExpandMailboxPattern(cReader, rule.Mailbox)
		if err != nil {
			lib.Log.Errorf(err.Error())
			continue
		}
		if len(mailboxNames) == 0 {
			lib.Log.DebugF("No mailboxes matched \"%s\"", rule.Mailbox)
			continue
		}
		...
		for _, mailboxName := range mailboxNames {
			...
			// Select mailbox
			mbox, err := cReader.Select(mailboxName, true)
			if err != nil {
				lib.Log.Errorf(err.Error())
				continue
			}
			...
			if mbox.Messages == 0 {
				lib.Log.DebugF("No messages matching search in %s", mailboxName)
				continue
			}
			...
			lib.Log.DebugF("Searching \"%s\" for %s", mailboxName, strings.Join(searchDescriptions, ", "))
			...
			if len(searchRes) <= 0 {
				lib.Log.DebugF("%s returned 0 results for %s", mailboxName, strings.Join(searchDescriptions, ", "))
				continue
			}

			_, err = cWriter.Select(mailboxName, false)
			if err != nil {
				lib.Log.Errorf(err.Error())
				continue
			}
			...
			for msg := range messages {
				...
				if doActions && (rule.RemoveAttachments() || rule.SaveAttachments()) {
					...
					if attachments > 0 && rule.RemoveAttachments() {
						...
						if err := cWriter.Append(mailboxName, msg.Flags, msg.Envelope.Date, literal); err != nil {
							lib.Log.Errorf(err.Error())
							continue
						}
					}
