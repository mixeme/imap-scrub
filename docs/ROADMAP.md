...
Tracked against [axllent/imap-scrub issues](https://github.com/axllent/imap-scrub/issues). This fork (`mixeme/imap-scrub`) continues development while upstream is being archived.
...
| Source | Summary | Status here |
| --- | --- | --- |
| [#2](https://github.com/axllent/imap-scrub/pull/2) | `export_mailbox` action → mbox backup | In release 0.1.0 — see [Changelog](../CHANGELOG.md) |
| [#14](https://github.com/axllent/imap-scrub/pull/14) | `older_than` uses IMAP internal date; local build scripts | In release 0.1.0 — see [Changelog](../CHANGELOG.md) |
| [#13](https://github.com/axllent/imap-scrub/pull/13) | `pass_file`, comma-separated `from`, `newer_than` | In release 0.1.0 — see [Changelog](../CHANGELOG.md) |
| [#7](https://github.com/axllent/imap-scrub/issues/7) / [#11](https://github.com/axllent/imap-scrub/pull/11) | Skip `remove_attachments` when message has no attachments | In upstream 0.0.6 |
| [#5](https://github.com/axllent/imap-scrub/issues/5) | Clarify `include_unread` behaviour | In upstream 0.0.4 |
| [#8](https://github.com/axllent/imap-scrub/pull/8) | Log saved attachment paths to `%d-attachments-deleted.txt` | In upstream 0.0.5 |
| [#1](https://github.com/axllent/imap-scrub/pull/1) | Fix hang on `Select()` for some mailboxes | Fixed in `lib/mailboxes.go` |
| [#12](https://github.com/axllent/imap-scrub/issues/12) | Replace removed attachment with stub file | Closed upstream — reporter confirmed existing behaviour is sufficient |
| [#3](https://github.com/axllent/imap-scrub/issues/3), [#4](https://github.com/axllent/imap-scrub/issues/4) | Gmail credentials / nil pointer panic | Closed upstream — bug reports, not open features |

### Maintenance

| Source | Summary | Notes |
| --- | --- | --- |
| [#10](https://github.com/axllent/imap-scrub/issues/10) | Project continuation | Upstream maintainer handed off to this fork; upstream to be archived with README pointer to `mixeme/imap-scrub`. |
